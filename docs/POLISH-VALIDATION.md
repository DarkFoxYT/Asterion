...
The Minotaur's procedural bone edits now clear GeckoLib's change markers after rendering. Unanimated channels therefore reset correctly instead of accumulating the previous frame's offsets. Grab aim stops following the held player back into its own hand, and both entities use interpolated positions. Clip sampling wraps looping animations and holds non-looping animations below their endpoint.
