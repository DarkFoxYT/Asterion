...
The throw deals 10 base damage and launches the player roughly 50–80 blocks over clear level ground. Server-side swept collision stops the flight at terrain, with another 10 base damage (five unarmored hearts) on wall impact. Damage still respects armor. The boss then attempts to pursue and pin the victim briefly against that wall; moving away can evade the follow-up. Debug mode supports this combo too. `/asterion minotaur attack grab` forces the initial attack; stand close enough to be caught.
