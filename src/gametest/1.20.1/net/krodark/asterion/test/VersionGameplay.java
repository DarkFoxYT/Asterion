...
final class VersionGameplay {
 static net.minecraft.world.inventory.CraftingContainer input(int width,int height,java.util.List<ItemStack> items) { var input=new net.minecraft.world.inventory.TransientCraftingContainer(new net.minecraft.world.inventory.AbstractContainerMenu(null,-1) {
   public net.minecraft.world.item.ItemStack quickMoveStack(net.minecraft.world.entity.player.Player p,int slot){return net.minecraft.world.item.ItemStack.EMPTY;}
   public boolean stillValid(net.minecraft.world.entity.player.Player p){return true;}
  },width,height);for(int i=0;i<items.size();i++)input.setItem(i,items.get(i));return input; }
 static ItemStack craft(ServerPlayer player,java.util.List<ItemStack> parts) {
  ...
  var shaped=(net.minecraft.world.item.crafting.CraftingRecipe)slab;
 ...
 static void verifySword(ItemStack sword) {
  var tag=net.krodark.asterion.port.compat.ItemData.get(sword,net.krodark.asterion.port.legacy.item.DataComponents.CUSTOM_DATA).copyTag();
  ...
  var model=net.krodark.asterion.port.compat.ItemData.get(sword,net.krodark.asterion.port.legacy.item.DataComponents.CUSTOM_MODEL_DATA);
  GameplaySmoke.require(model.value()==25,"Blade/guard/pommel model selection changed");
 }
 static void roof(ServerPlayer player) {
  net.krodark.asterion.port.legacy.network.ServerPlayNetworking.send(player,new net.krodark.asterion.network.RoofCollapsePayload(new net.minecraft.world.phys.Vec3(0,178,3),180));
 }
 static void advancement(ServerPlayer player,String name) {
  var advancement=player.server.getAdvancements().getAdvancement(Asterion.id(name));
