...
public final class UnderwaterRuinFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        if (context.random().nextInt(LabyrinthConfig.INSTANCE.underwaterRuinChance) != 0) return false;
        BlockPos center = context.origin();
        if (!context.level().getBlockState(center.above(3)).is(Blocks.WATER)) return false;

        var template = context.level().getLevel().getStructureManager().get(Labyrinth.id("underwater_ruin"));
        if (template.isEmpty()) {
            Labyrinth.LOGGER.error("Missing structure template labyrinth:underwater_ruin; run rebuildStructureTemplates");
            return false;
        }
        ...
        if (context.level().getBlockEntity(barrelPos) instanceof BarrelBlockEntity barrel) {
            ...
            if (context.random().nextFloat() < LabyrinthConfig.INSTANCE.mechanismChance)
                barrel.setItem(13, Labyrinth.ANTIKYTHERA_MECHANISM.getDefaultInstance());
            barrel.setChanged();
        }
        return true;
    }
