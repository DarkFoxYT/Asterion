...
final class WebPhysicsGraph {
    ...
    private void build(){ int index=0;for(int edge=0;edge<patch.edges().size();edge++){WebPatch.Edge e=patch.edges().get(edge);Vec3 a=patch.anchors().get(e.a()),b=patch.anchors().get(e.b());int previous=e.a();int pieces=patch.pieces(edge);double rest=a.distanceTo(b)/pieces*1.008;for(int i=1;i<pieces;i++){Vec3 point=a.lerp(b,i/(double)pieces).add(0,-Math.sin(Math.PI*i/pieces)*.025,0);int n=p.size();p.add(point);old.add(point);pinned.add(false);links.add(new Link(previous,n,rest,edge,index++));previous=n;}links.add(new Link(previous,e.b(),rest,edge,index++));} }
    void step(Level level,Vec3 player,Vec3 playerVelocity,double radius,java.util.BitSet cuts){
        ...
        for(int substep=0;substep<2;substep++){
            for(int i=0;i<p.size();i++)if(!pinned.get(i)){Vec3 current=p.get(i),velocity=current.subtract(old.get(i)).scale(.985),next=current.add(velocity).add(0,-.0015,0);old.set(i,current);p.set(i,collide(level,current,next));}
            ...
            for(int pass=0;pass<16;pass++)for(Link link:links)if(!cuts.get(link.index))project(link);
