...
public final class LimboWaterRenderer {
    ...
    private record Tile(int x, int z, int[] vertices, int[] fineVertices, int[] shore, long refreshed) { }
    private LimboWaterRenderer() { }

    public static boolean replacesSurface(BlockAndTintGetter level, BlockPos pos) {
        return enabled && pos.getY() == UnderworldTerrain.WATER_Y && surface(level, pos);
    }
    ...
    public static void initialize() {
        ...
        LevelRenderEvents.END_EXTRACTION.register(context -> {
            ...
            for (int x = cx - radius; x <= cx + radius; x++) for (int z = cz - radius; z <= cz + radius; z++) {
                ...
                if (frustum != null && !frustum.isVisible(new AABB(x * 16, UnderworldTerrain.WATER_Y - 4,
                        z * 16, x * 16 + 16, UnderworldTerrain.WATER_Y + 5, z * 16 + 16))) continue;
                ...
                if (tile == null || (refreshBudget > 0 && level.getGameTime() - tile.refreshed > 100)) {
                    if (tile != null) refreshBudget--;
                    tile = topology(level, x * 16, z * 16); TILES.put(key, tile);
                }
                if (tile.vertices.length != 0) next.add(tile);
            }
        ...
        LevelRenderEvents.BEFORE_TRANSLUCENT_TERRAIN.register(context -> {
            ...
            for (Tile tile : frame) {
                ...
                for (int vertex : fine ? tile.fineVertices : tile.vertices) {
                    ...
                    float boatHeight = frameBoat == null ? 0 : (float)Math.clamp(
                            (frameBoat.y - UnderworldTerrain.WATER_Y - 8.0 / 9.0) / 8, -1, 1);
                    out.addVertex(pose, (float)(x - camera.x),
                                    (float)(UnderworldTerrain.WATER_Y + 8.0 / 9.0 - camera.y), (float)(z - camera.z))
                            .setUv(x, z).setUv2(timeLow, timeHigh)
                            .setUv1(bx, bz).setNormal(boatCos, boatHeight, boatSin)
                            .setColor(tile.shore[vertex], fraction,
    ...
    private static Tile topology(ClientLevel level, int x, int z) {
        // Each column is checked once, then shared by its four surrounding vertices.
        int[] depths = new int[34 * 34];
        var pos = new BlockPos.MutableBlockPos();
        var below = new BlockPos.MutableBlockPos();
        for (int dz = 0; dz < 34; dz++) for (int dx = 0; dx < 34; dx++) {
            pos.set(x + dx - 9, UnderworldTerrain.WATER_Y, z + dz - 9);
            if (!level.getChunkSource().hasChunk(pos.getX() >> 4, pos.getZ() >> 4) || !surface(level, pos)) continue;
            // The outer margin is only a land-distance mask; never scan its seabed.
            int depth = dx < 8 || dx > 25 || dz < 8 || dz > 25 ? 6 : 1;
            while (depth < 6 && level.getFluidState(below.set(pos.getX(), pos.getY() - depth, pos.getZ())).is(FluidTags.WATER)) depth++;
            depths[dz * 34 + dx] = depth;
        }
        boolean[] wet = new boolean[256];
        boolean any = false;
        for (int i = 0; i < wet.length; i++) {
            wet[i] = depths[(i / 16 + 9) * 34 + i % 16 + 9] > 0;
            any |= wet[i];
        }
        if (!any) return new Tile(x, z, new int[0], new int[0], new int[0], level.getGameTime());
        int[] shore = new int[17 * 17];
        for (int dz = 0; dz <= 16; dz++) for (int dx = 0; dx <= 16; dx++) {
            shore[dz * 17 + dx] = Math.round(255 * net.krodark.asterion.update.underworld.world.WaterShoreline.attenuation(depths, 34, dx + 9, dz + 9));
        }
        return new Tile(x, z, WaterSurfaceMesh.vertices(wet, shore), WaterSurfaceMesh.vertices(wet, new int[289]), shore, level.getGameTime());
    }
}
