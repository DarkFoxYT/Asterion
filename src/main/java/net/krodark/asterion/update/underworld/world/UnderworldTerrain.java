...
public final class UnderworldTerrain {
        ...
        for (int x = chunk.getPos().getMinBlockX(); x <= chunk.getPos().getMaxBlockX(); x++) {
            for (int z = chunk.getPos().getMinBlockZ(); z <= chunk.getPos().getMaxBlockZ(); z++) {
                ...
                int ceilingSpike = c.spider && c.roof - c.floor > 13 && d.hanging > 1
                        ...
                        Math.max(0, c.roof - c.floor - 5 - d.rock - d.spike - d.hanging)) : 0;
                Column[] neighbors = c.spider && !c.path && c.roof - c.floor > 10 && (texture & 3L) == 0
                        ? new Column[]{column(seed, x - 1, z, caveNodes), column(seed, x + 1, z, caveNodes),
                        column(seed, x, z - 1, caveNodes), column(seed, x, z + 1, caveNodes)} : null;
                ...
                for (int y = MIN_Y; y <= MAX_Y; y++) {
                    ...
                    if (c.open && y > MIN_Y && y < MAX_Y) {
                        ...
                        if (neighbors != null && state.isAir() && y > c.floor + 3 && y < c.roof - 3) {
                            boolean wallContact = false;
                            for (Column neighbor : neighbors) if (!neighbor.open || y <= neighbor.floor || y >= neighbor.roof) {
                                wallContact = true;
                                break;
                            }
                            if (wallContact && octaves(seed ^ 0xA11FA5L,
                                    x * .16 + y * .055, z * .16 - y * .043) > .16)
                                state = (texture & 8L) == 0 ? stone : shale;
                        }
                        // Silk is generated by WebPatchGenerator and rendered as strands.
                        ...
                        if (pool <= 1.3 && Math.abs(offset) > 4 && !c.path && !c.spider) {
                            ...
                            if (pool <= 1.03 && y == poolY) state = Blocks.AIR.defaultBlockState();
    ...
    private static Column column(long seed, int x, int z, java.util.Map<Long, CaveNode> caveNodes) {
        ...
        double shoulder = smooth((Math.abs(offset + 15) - 4) / 10);
        double floor = pathFloor(z) + shoulder * 3;
        if (tunnel && z < 18 && !path) {
            double roughPatch = smooth((octaves(seed ^ 0xFA17EL, x * .027, z * .027) + .12) / .38);
            double safeLane = smooth((Math.abs(offset) - 3) / 5);
            floor += shoulder * safeLane * (2.7 * octaves(seed ^ 0xF1007L, x * .055, z * .055)
                    + (.4 + roughPatch * 1.3) * octaves(seed ^ 0xF1A6EL, x * .19, z * .19));
        }
        ...
        if (side.open && z < -36) {
            ...
            roof = tunnel ? Math.max(roof, side.roof) : side.roof;
        }
        if (spider.open) {
            open = true;
            floor = tunnel || side.open ? Math.min(floor, spider.floor) : spider.floor;
            roof = tunnel || side.open ? Math.max(roof, spider.roof) : spider.roof;
        }
    ...
    private static SideShape spiderCave(long seed, int x, int z, java.util.Map<Long, CaveNode> nodes) {
        ...
        double rim = Math.clamp(clearance / 9, 0, 1);
        double round = Math.sqrt(Math.max(0,1-(1-rim)*(1-rim)));
        ...
        // Floor and roof meet at the rim, forming rounded shoulders instead of a vertical cut.
        double equator=base+height*.32;
        int floor = (int)Math.floor(equator - height * round * .52 - voidStrength * 70);
        int roof = (int)Math.ceil(equator + height * round * .70 + voidStrength * 75);
    ...
    private static SideShape sideShape(int x, int z) {
        ...
        int base=pathFloor(z);
        double radial=dx*dx+dz*dz;
        double arc=Math.sqrt(Math.max(0,1-radial));
        double height=nestSlot(b.slot)?24:14;
        int floor=room?(int)Math.floor(base+height*.28*(1-arc)):base;
        int roof=room?(int)Math.ceil(base+height*.28+height*.72*arc):base;
        if(hall) {
            double cross=(x-passageX)/width;
            double hallArc=Math.sqrt(Math.max(0,1-cross*cross));
            floor=Math.min(floor,(int)Math.floor(base+2*(1-hallArc)));
            roof=Math.max(roof,(int)Math.ceil(base+2+7*hallArc));
        }
