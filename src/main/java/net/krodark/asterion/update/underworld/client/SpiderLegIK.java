...
public final class SpiderLegIK {
    ...
    public record Debug(List<Leg> legs, float age) { }
    public static Debug debug(LimboSpiderEntity spider) { return DEBUG.get(spider); }

    /** Small visual lean from planted feet, expressed in the un-tilted body frame. */
    static Vec3 bodyTilt(Debug debug,Quaternionf orientation) {
        ...
        if(feet.size()<4)return Vec3.ZERO;
        ...
        double pitch=-Math.atan(b)*.25,roll=Math.atan(a)*.25;
        double length=Math.hypot(pitch,roll),limit=Math.toRadians(4);
    ...
    public static void apply(RenderPassInfo<EntityRenderState> pass, BoneSnapshots bones, Frame frame) {
        ...
        for (int leg = 0; leg < 8; leg++) {
            ...
            // mirrored by GeckoLib's Bedrock importer). Keep all authored segment lengths.
            ...
            Vector3f tip = new Vector3f(pivots[2]).add(leg < 4 ? -15F/16 : 15F/16, -6F/16, 0);
            Vec3 nominal = vec(world.transformPosition(endpoint(pivots,angles,tip)));
            Vec3 start = nominal.add(probeUp.scale(1.2));
            Vec3 end = nominal.subtract(probeUp.scale(1.6));
            var hit = frame.spider.level().clip(new ClipContext(start,end,ClipContext.Block.COLLIDER,
                    ClipContext.Fluid.NONE,frame.spider));
            boolean contact = hit.getType() != HitResult.Type.MISS;
            Vec3 contactNormal = contact ? hit.getDirection().getUnitVec3() : probeUp;
            Vec3 target = contact ? hit.getLocation().add(contactNormal.scale(.035)) : nominal;
            if(!contact) {
                ...
                var nearby=frame.spider.level().clip(new ClipContext(inset.add(probeUp.scale(1.2)),
                        inset.subtract(probeUp.scale(1.6)),ClipContext.Block.COLLIDER,ClipContext.Fluid.NONE,frame.spider));
            ...
            Vec3 silk = LimboWebWorldRenderer.spiderContact(nominal, .7);
            ...
            if (update) {
                ...
                boolean turn = ((int)(gaitClock / 4) & 1) == ((leg + leg / 4) & 1);
                ...
                boolean supported = old == null || memory.destinations[leg] != null
                        ...
                        || LimboWebWorldRenderer.spiderContact(old,.2) != null;
                if(supported)memory.unsupportedSince[leg]=-1;
                else if(memory.unsupportedSince[leg]<0)memory.unsupportedSince[leg]=frame.age;
                boolean released=!supported && frame.age-memory.unsupportedSince[leg]>=2;
                if (reset || old == null || released || old.distanceToSqr(target) > 3.24) {
                    memory.feet[leg] = target;
                    memory.destinations[leg] = null;
                    memory.contactNormals[leg]=contactNormal;
                } else {
                    if (contact && memory.destinations[leg] == null && old.distanceToSqr(target) > (moving ? .16 : .06)
                    ...
                    if (memory.destinations[leg] != null) {
                        double t = Math.clamp((frame.age-memory.stepStart[leg])/3.5,0,1);
                        ...
                        var obstacle=frame.spider.level().clip(new ClipContext(swing.add(probeUp.scale(1.25)),
        ...
        DEBUG.put(frame.spider,new Debug(List.copyOf(debug),frame.age));
    }
    ...
    static Vec3 swingFoot(Vec3 from,Vec3 to,Vec3 up,double t) {
        t=Math.clamp(t,0,1);
        double highest=Math.max(from.dot(up),to.dot(up))+.18;
