...
public final class BetterLightningRenderer {
    ...
    public static void draw(VertexConsumer out, PoseStack.Pose pose, Vec3 camera,
        ...
        int quality = config.enhancedLightning ? config.cinematicQuality : 0;
        int subdivisions = quality == 0 ? (distance > 72.0D ? 5 : 4)
                : distance > 120.0D ? 6 : distance > 48.0D ? 5 : 4;
        int branchCount = quality == 0
                ? Mth.clamp(2 + Mth.floor(distance / 36.0D), 2, 6)
                : quality == 1 ? Mth.clamp(5 + Mth.floor(distance / 18.0D), 5, 14)
                : Mth.clamp(8 + Mth.floor(distance / 13.0D), 8, 22);
        ...
        float powerScale = quality == 0 ? 1.45F : quality == 1 ? 2.15F : 2.85F;
        float width = distanceWidth * (0.66F + charge * 0.34F) * powerScale;
        ...
        drawPath(out, pose, camera, spine, width * 1.82F, coronaR, coronaG, coronaB, 82, true, frame, 0);
        drawPath(out, pose, camera, spine, width, 255, Math.max(8, coreG), Math.max(4, coreB), 236, true, frame, 1);
        drawPath(out, pose, camera, spine, width * 0.24F, 255, 218, 188, 255, true, frame, 2);
        for (BoltBuilder.Branch branch : branches) {
            drawPath(out, pose, camera, branch.points(), width * 0.68F * branch.strength(),
                    coronaR, coronaG, coronaB, 148, true, frame, 3);
            drawPath(out, pose, camera, branch.points(), width * 0.19F * branch.strength(),
                    255, Math.max(8, coreG), Math.max(4, coreB), 248, false, frame, 4);
        }
    }
