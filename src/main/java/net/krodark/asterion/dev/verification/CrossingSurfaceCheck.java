...
final class CrossingSurfaceCheck {
    static void run(ServerLevel level) {
        ...
        for(String name:new String[]{"crossing_01","crossing_02"})for(Rotation rotation:Rotation.values()) {
            BlockPos origin=new BlockPos(3+index++*32,19,131);
            ...
            for(int x=0;x<19;x++)for(int z=0;z<19;z++) {
                ...
                for(int y=48;y<=floor;y++)level.setBlock(new BlockPos(wx,y,wz),Blocks.STONE.defaultBlockState(),18);
                // Two flanking walls and a wall directly above the hatch recess.
                if(x==4 || x==14 || x>=8 && x<=10 && z==9)for(int y=floor+1;y<=65;y++) {
            ...
            for(var chunk:chunks) {
                var clip=new BoundingBox(chunk.getMinBlockX(),19,chunk.getMinBlockZ(),chunk.getMaxBlockX(),49,chunk.getMaxBlockZ());
            ...
            var settings=AuthoredCatacombs.settings(new BoundingBox(origin.getX(),19,origin.getZ(),origin.getX()+18,49,origin.getZ()+18))
