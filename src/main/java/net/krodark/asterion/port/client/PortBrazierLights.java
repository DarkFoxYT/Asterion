...
public final class PortBrazierLights {
    ...
    private PortBrazierLights() { }

    public static void reset() { cooldown = 0; }
    ...
    public static void tick(Minecraft client) {
        ...
        for (BlockPos pos : fixtures) {
            ...
            if (state.is(Asterion.LAMENTER) && state.getValue(LamenterBlock.CRYING)) {
                PortLight.updateItemGlowLight(pos, new Vec3(pos.getX() + .5, pos.getY() + .72, pos.getZ() + .5),
                        .18F, .72F, 1F, .72F, 4.25F, false);
            } else if (state.is(Asterion.GREEK_BRAZIER) && GreekBrazierBlock.isRoot(state)
                    && state.getValue(BlockStateProperties.LIT)) {
                PortLight.updateItemGlowLight(pos, new Vec3(pos.getX() + .5, pos.getY() + 1.35, pos.getZ() + .5),
