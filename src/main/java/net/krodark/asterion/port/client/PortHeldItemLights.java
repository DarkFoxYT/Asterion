...
public final class PortHeldItemLights {
    ...
    public static void tick(Minecraft client) {
        ...
        for (Object key : ACTIVE_LIGHTS) {
            if (!currentLights.contains(key)) {
                PortLight.removeItemGlowLight(key);
            }
        }
        ACTIVE_LIGHTS.clear();
    ...
    private static void trackHand(AbstractClientPlayer player, InteractionHand hand,
        ...
        if (PortRagdolls.isRagdolled(player)) {
            ...
            Vec3 handPosition = PortRagdolls.ragdollHandPosition(player.getId(), physicalRight);
    ...
    public static void renderFrame(Minecraft client, float partialTick) {
        ...
        for (AbstractClientPlayer player : client.level.players()) {
            if (!player.isAlive() || player.distanceToSqr(client.player) > rangeSquared
                    || PortRagdolls.isRagdolled(player)) continue;
    ...
    private static void updateAt(Object key, net.minecraft.world.entity.Entity owner, Vec3 position,
        ...
        PortLight.updateItemGlowLight(key, position, style.red, style.green, style.blue,
    ...
    public static void clear() {
        ACTIVE_LIGHTS.forEach(PortLight::removeItemGlowLight);
        ACTIVE_LIGHTS.clear();
