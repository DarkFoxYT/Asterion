...
public final class PortMazeObjectiveOverlay {
    ...
    private PortMazeObjectiveOverlay() { }

    public static void register() {
        net.fabricmc.fabric.api.client.rendering.v1.HudRenderCallback.EVENT.register((graphics, delta) -> PortMazeObjectiveOverlay.render(graphics, Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(false)));
    }
    ...
    public static void tick(Minecraft client) {
        ...
        boolean tumbling = PortRagdolls.isRagdolled(client.player);
        if (armed) {
            waitTicks++;
            sawTumble |= tumbling;
    ...
    private static void render(GuiGraphics graphics, float partialTick) {
        if (!visible || sharedStage < 0 || sharedStage == Stage.values().length || PortCinematicHud.isHidden() || !AsterionConfig.INSTANCE.objectiveHudEnabled) return;
        ...
        float renderTicks = visibleTicks + Mth.clamp(partialTick, 0.0F, 1.0F);
        ...
        int textLeft = left + 9;
        graphics.drawString(client.font, INTRO, textLeft, panelTop + 5,
                Math.round(alpha * 0.74F) << 24 | 0xC18468, false);
        graphics.drawString(client.font, progressLabel, left + panelWidth - 8 - progressWidth, panelTop + 5,
                Math.round(alpha * 0.58F) << 24 | 0xA89185, false);
        graphics.drawString(client.font, objectiveLine, textLeft, panelTop + 17,
                alpha << 24 | 0xF2DED0, false);
        for (int line = 0; line < hintLines.size(); line++)
            graphics.drawString(client.font, hintLines.get(line), textLeft, panelTop + 29 + line * 9,
        ...
        if (waypoint != null) {
            ...
            graphics.drawString(client.font, Component.literal(arrow), textLeft, panelTop + waypointY,
                    alpha << 24 | 0xE8B94A, false);
            graphics.drawString(client.font, waypointText, textLeft + 13, panelTop + waypointY,
