...
public final class PortBossFinaleOverlay {
    ...
    private PortBossFinaleOverlay() { }

    public static void register() {
        net.fabricmc.fabric.api.client.rendering.v1.HudRenderCallback.EVENT.register((graphics, delta) -> render(graphics, Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(false)));
    }
    ...
    private static void render(GuiGraphics graphics, float partialTick) {
        ...
        if (overworldReady) {
            ...
            if (fadeTicks < CREDITS_TICKS) {
                float time = fadeTicks + partialTick;
    ...
    public static void finish(Minecraft client) {
        if (active) PortAudio.endCredits();
        active = false;
        overworldReady = false;
        fadeTicks = 0;
        ticks = 0;
        ...
        PortDeadSunEvents.clearTransientEffects();
    }
