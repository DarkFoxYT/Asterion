...
public final class PortDimensionEffects {
    ...
    public static void initialize() {
        ...
        PostEffects.register(Asterion.id("dimension/dead_sun"), config -> config
                ...
                .uniformVec4("DeadSunData", PortDimensionEffects::deadSunData)
                .uniformVec4("DeadSunTuning", PortDimensionEffects::deadSunTuning)
                .uniform("EclipseData", PortDeadSunEvents::eclipse)
                .uniform("WorldDarkness", PortDeadSunEvents::darkness)
                .uniform("EntryRadiance", PortDimensionEffects::radiance)
                .uniform("FinaleProgress", PortBossFinaleOverlay::sunDetonationStrength)
    ...
    private static Vector4f deadSunData() {
        ...
        Vec3 shake = PortDeadSunEvents.sunOffset();
    ...
    private static Vector4f deadSunTuning() {
        ...
        float finale = PortBossFinaleOverlay.sunDetonationStrength();
        return new Vector4f(
                config.deadSunBrightness * mix(1.0F, 0.95F, eclipse)
                        * mix(1.0F, 1.18F, PortDeadSunEntryCinematic.radianceStrength())
                        * mix(1.0F, 0.52F, finale),
                config.shaderAnimationSpeed * mix(1.0F, 0.72F, eclipse),
                config.deadSunCorona * mix(1.0F, 2.15F, eclipse)
                        * mix(1.0F, 1.22F, PortDeadSunEntryCinematic.radianceStrength())
    ...
    public static void tickBiomeAtmosphere(Minecraft client) {
        if (!isInsideAsterion() || client.player == null) {
            ...
            float floodTarget = PortDeadSunEvents.floodStrength();
    ...
    private static float eclipse() {
        return Math.max(0.0F, Math.min(1.0F, PortDeadSunEvents.eclipse()));
    }

    private static float darkness() {
        return Math.max(0.0F, Math.min(1.0F, PortDeadSunEvents.darkness()));
    }

    private static float radiance() {
        return Math.max(PortDeadSunEntryCinematic.radianceStrength(),
                PortBossFinaleOverlay.sunDetonationStrength());
    }
