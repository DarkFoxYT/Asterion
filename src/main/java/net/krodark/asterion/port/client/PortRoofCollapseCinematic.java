...
public final class PortRoofCollapseCinematic {
    ...
    private PortRoofCollapseCinematic() { }
    ...
    public static void begin(RoofCollapsePayload payload) {
        ...
        PortCinematicHud.begin(client);
        client.levelRenderer.allChanged();
    }

    public static void tick(Minecraft client) {
        if (net.krodark.asterion.port.client.ragdoll.RagdollCompat.isPlayback(client)) {
        ...
        if (showShot) {
            PortCinematicHud.maintain(client);
            client.options.setCameraType(CameraType.FIRST_PERSON);
            client.smartCull = false;
        }
    }
    ...
    public static CameraPose cameraPose(Vec3 playerEye, float partial) {
        ...
        Vec3 wide = center.add(outward.scale(22.0D)).add(side.scale(-6.0D)).add(0, 15.0D, 0);
        Vec3 impact = center.add(outward.scale(17.0D)).add(side.scale(4.0D)).add(0, 10.0D, 0);
        Vec3 revival = center.add(outward.scale(14.0D)).add(side.scale(-1.5D)).add(0, 7.0D, 0);
