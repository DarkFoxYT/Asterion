...
/** Restores eclipse/rumble presentation state consumed by camera and Amnetic. */
public final class PortDeadSunEvents {
    ...
    private static int wardDarknessTicks, wardDarknessDuration;
    ...
    public static void clearTransientEffects() { active = null; rumbles.clear(); warnings.clear(); wardDarknessTicks = 0; }
    public static void receiveWardZap(net.krodark.asterion.network.MazeZapPayload payload) {
        var client=Minecraft.getInstance();
        if(client.player == null || payload.targetEntityId()!=client.player.getId()) return;
        wardDarknessTicks=wardDarknessDuration=Math.max(40,payload.durationTicks()+24);
    }
    public static boolean isCrimsonFireflyEventActive() {
        return active != null && active.id.equals(DeadSunEventSystem.CRIMSON_FIREFLIES);
    }

    public static float darkness() {
        if(wardDarknessTicks<=0 || wardDarknessDuration<=0) return eclipse();
        float fadeIn=Mth.clamp((wardDarknessDuration-wardDarknessTicks)/8F,0,1);
        float fadeOut=Mth.clamp(wardDarknessTicks/16F,0,1);
        return Math.max(eclipse(),smooth(fadeIn)*smooth(fadeOut));
    }
