...
import net.krodark.asterion.port.client.PortPerformanceGovernor;
import net.krodark.asterion.port.client.PortLight;
import net.krodark.asterion.port.client.PortDeadSunEvents;
import net.krodark.asterion.port.client.PortDimensionEffects;
...
public final class FlyingInsectParticle extends TextureSheetParticle {
    ...
    private void steerTowardLight() {
        if (--lightScanTicks <= 0) {
            lightTarget = PortLight.nearestAttractor(new Vec3(x, y, z), 14.0D);
    ...
    private void updateGlow(boolean enraged) {
        ...
        if (config.dynamicLightsEnabled && ownsAmneticLight) {
            PortLight.updateItemGlowLight(this, new Vec3(x, y, z),
            ...
            PortLight.removeItemGlowLight(this);
            ownsDynamicLight = false;
        }
        int effectiveQuality = Math.min(config.ambientParticleQuality, PortPerformanceGovernor.quality());
    ...
    private static boolean isEnraged() {
        return PortDeadSunEvents.isCrimsonFireflyEventActive()
                && PortDimensionEffects.isCrimsonBiome();
    }

    @Override public void remove() {
        if (firefly) {
            ACTIVE_FIREFLIES.remove(this);
            if (ownsDynamicLight) PortLight.removeItemGlowLight(this);
        }
        super.remove();
    }

    @Override protected int getLightColor(float partialTick) {
        return firefly ? 0xF000F0 : super.getLightColor(partialTick);
    }
