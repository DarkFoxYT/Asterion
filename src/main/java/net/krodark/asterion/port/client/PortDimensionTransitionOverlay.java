...
public final class PortDimensionTransitionOverlay {
    ...
    private PortDimensionTransitionOverlay() {
    }

    public static void register() {
        net.fabricmc.fabric.api.client.rendering.v1.HudRenderCallback.EVENT.register((graphics, delta) -> PortDimensionTransitionOverlay.renderHud(graphics, Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(false)));
    }
    ...
    private static boolean destinationChunksReady(Minecraft client) {
        ...
        int radius = PortDeadSunEntryCinematic.requiredChunkRadius();
    ...
    private static void clear() {
        if (deathMessage == 0) {
            PortDeadSunEntryCinematic.begin();
            PortMazeObjectiveOverlay.armAfterArrival();
        }
        deathMessage = 0;
        stableTicks = 0;
        fadeOutProgress = 0;
    ...
    public static void renderLoadingScreen(GuiGraphics graphics) {
        graphics.fill(0, 0, graphics.guiWidth(), graphics.guiHeight(), 0xFF000000);
        if (PortBossFinaleOverlay.isActive()) return;
        renderDescent(graphics, 255);
    }

    private static void renderHud(GuiGraphics graphics, float partialTick) {
        if (!active) return;
        int alpha = fadingOut
    ...
    private static void renderDescent(GuiGraphics graphics, int alpha) {
        Minecraft client = Minecraft.getInstance();
        if (deathMessage != 0) {
            graphics.drawCenteredString(client.font, Component.translatable(deathMessage == 1
        ...
        for (int index = 0; index < text.length(); index++) {
            ...
            graphics.drawCenteredString(client.font, Component.literal(letter),
