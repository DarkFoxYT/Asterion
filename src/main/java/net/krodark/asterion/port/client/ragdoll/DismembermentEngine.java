...
public final class DismembermentEngine {
    ...
    private final Map<Integer, RagdollCompat.Skin> playerSkins = new HashMap<>();
    ...
    public boolean ragdoll(Entity entity, int impactRegion, Vec3 point, Vec3 direction,
        ...
        if (entity instanceof net.minecraft.client.player.AbstractClientPlayer player) {
            ...
            playerSkins.put(entity.getId(), RagdollCompat.skin(player));
        ...
        if (requestServerKill && !net.krodark.asterion.port.client.ragdoll.RagdollCompat.isPlayback(Minecraft.getInstance())
    ...
    private void spawn(Entity entity, int region, int parentRegion, Vec3 impact, Vec3 direction,
                       double force, boolean separated, ResourceLocation texture) {
    ...
    private void spawnCape(Entity entity) {
        ...
        if (!(entity instanceof net.minecraft.client.player.AbstractClientPlayer player)
                || RagdollCompat.skin(player).cape() == null
        ...
        RigidBodyPiece cape = new RigidBodyPiece(entity.getId(), 6, 1, true,
                ...
                (float) center.distanceTo(torso.position), RagdollCompat.skin(player).cape(),
    ...
    private static Quaternionf vanillaElytraPose(Player player, boolean left, double sign) {
        try {
            ...
            model.setupAnim(player, 0, 0, player.tickCount, player.getYHeadRot()-player.yBodyRot, player.getXRot());
    ...
    private void applyPlayerGlobalFlightPose(Entity entity) {
        ...
        try {
            ...
            float flightScale = Mth.clamp(player.getFallFlyingTicks()*player.getFallFlyingTicks()/100.0F,0,1);

            float baseYaw = (float) Math.toRadians(180.0f - player.yBodyRot);
            float flightPitch = (float) Math.toRadians(
                    flightScale * (-90.0f - player.getXRot()));
            Quaternionf base = new Quaternionf().rotationY(baseYaw);
            Quaternionf flight = new Quaternionf().rotationX(flightPitch);
            Vec3 velocity=player.getDeltaMovement(), look=player.getViewVector(1);
            double horizontal=velocity.horizontalDistanceSqr()*look.horizontalDistanceSqr();
            if(horizontal>1.0E-6) {
                double cosine=Mth.clamp((velocity.x*look.x+velocity.z*look.z)/Math.sqrt(horizontal),-1,1);
                flight.rotateY((float)(Math.signum(velocity.x*look.z-velocity.z*look.x)*Math.acos(cosine)));
            }
    ...
    private void synchronizePlayerAttachments(ClientLevel level) {
        for (int entityId : new HashSet<>(ragdolled)) {
            ...
            boolean capeVisible = RAGDOLL_CAPE_ENABLED
                    ...
                    && RagdollCompat.skin(clientPlayer).cape() != null
    ...
    private static double armorItemWeight(net.minecraft.world.item.ItemStack stack) {
        ...
        return stack.getItem() instanceof net.minecraft.world.item.ArmorItem ? 0.095 : 0.0;
    }
    ...
    private static BodyGeometry calculateGeometry(Entity entity, int region,
        ...
        try {
            ...
            if (applyAnimation) {
                RagdollCompat.setup(model, entity);
            }
            if (cubes.isEmpty()) RagdollCompat.root(model).visit(new PoseStack(), (pose, path, index, cube) ->
                    cubes.add(new ModelCube(cube, RagdollCompat.copyPose(pose), path, transformedBounds(pose, cube))));
            ...
            if (playerGeometry && model instanceof HumanoidModel humanoid) {
                ...
                ModelPart node = RagdollCompat.root(model);
            ...
            if (playerGeometry) {
                ...
                float bodyYaw = ((Player) entity).yBodyRot;
    ...
    private static List<RigidBodyPiece.ModelBox> captureCustomPlayerBoxes(Entity entity, int region,
        ...
        var frame = new org.joml.Matrix4f().translation((float)center.x, (float)center.y, (float)center.z)
                ...
                .rotateY((float)Math.toRadians(180 - ((Player)entity).yBodyRot))
    ...
    private void captureFreshPlayerPose(Player player) {
        ...
        if (renderer instanceof LivingEntityRenderer living) {
            EntityModel model = living.getModel();
            RagdollCompat.setup(model, player);
    ...
    private static float[][] resolveFaceUvs(Entity entity, int region) {
        try {
            ...
            if (renderer instanceof LivingEntityRenderer<?, ?> living) {
                ...
                for (Object value : RagdollCompat.root(model).getAllParts().toList()) {
    ...
    private static float[][] buildUvFaces(ModelPart.Cube cube) {
        ...
        for (ModelPart.Polygon polygon : ((net.krodark.asterion.mixin.PortModelCubeAccessor)(Object)cube).asterion$polygons()) {
            ...
            for (ModelPart.Vertex vertex : polygon.vertices) {
                int corner = (Math.abs(vertex.pos.x() - cube.maxX) < Math.abs(vertex.pos.x() - cube.minX) ? 1 : 0)
                        | (Math.abs(vertex.pos.y() - cube.maxY) < Math.abs(vertex.pos.y() - cube.minY) ? 2 : 0)
                        | (Math.abs(vertex.pos.z() - cube.maxZ) < Math.abs(vertex.pos.z() - cube.minZ) ? 4 : 0);
    ...
    private static float[][] capeFaceUvs() {
        try {
            ModelPart cape = net.minecraft.client.model.PlayerModel.createMesh(net.minecraft.client.model.geom.builders.CubeDeformation.NONE,false).getRoot().bake(64,64).getChild("cloak");
    ...
    @SuppressWarnings({"rawtypes", "unchecked"})
    private static ResourceLocation resolveTexture(Entity entity) {
        if (entity instanceof net.minecraft.client.player.AbstractClientPlayer player) return RagdollCompat.skin(player).texture();
        if (entity instanceof ConstructEntity) return Asterion.id("textures/entity/construct.png");
        try {
            EntityRenderer renderer=Minecraft.getInstance().getEntityRenderDispatcher().getRenderer(entity);
            return renderer.getTextureLocation(entity);
        } catch(RuntimeException error) { return ResourceLocation.withDefaultNamespace("textures/block/red_concrete.png"); }
    }
    ...
    public void forcePlayerTumble(Minecraft client, Vec3 sourcePosition, Vec3 impulse, float force) {
        if (net.krodark.asterion.port.client.ragdoll.RagdollCompat.isPlayback(client)) return;
    ...
    public void releaseRagdoll(int entityId) {
        ...
        if (net.krodark.asterion.port.client.ragdoll.RagdollCompat.isPlayback(client)) { removeRagdoll(entityId); return; }
    ...
    private void sendPoseSnapshots() {
        if (net.krodark.asterion.port.client.ragdoll.RagdollCompat.isPlayback(Minecraft.getInstance())) return;
    ...
    public Vec3 heldRenderOffset(int entityId, float partial) {
        ...
        Vec3 feet = net.krodark.asterion.port.client.ragdoll.MinotaurHandAttachment.feet(player);
    ...
    private void synchronizePlayerSkins(ClientLevel level) {
        ...
        for (var entry : playerSkins.entrySet()) {
            ...
            var skin = RagdollCompat.skin(player);
    ...
    private boolean preparePlayback(Minecraft client) {
        if (!net.krodark.asterion.port.client.ragdoll.RagdollCompat.isPlayback(client) || client.level == null) return false;
    ...
    public void tick(ClientLevel level, Entity collisionContext) {
        if (net.krodark.asterion.port.client.ragdoll.RagdollCompat.isPlayback(Minecraft.getInstance())) {
