...
/** Rune renderer with the original powered fade restored through Amnetic bloom. */
public final class PortRuneRenderer extends SimpleGeoBlockRenderer<RuneBlockEntity> {
    ...
    private static int glowColor(RuneBlockEntity rune) {
        int alpha = net.krodark.asterion.port.compat.MathCompat.clamp(Math.round(rune.glowPercent() * 2.55F), 0, 255);
