...
public final class SavedDataCompat {
    ...
    public record Factory<T extends SavedData>(Codec<T> codec, Supplier<T> fallback) {}
    public static <T extends SavedData> Factory<T> factory(Codec<T> codec, Supplier<T> fallback) {
        return new Factory<>(codec, fallback);
    }
    public static <T extends SavedData> T get(net.minecraft.world.level.storage.DimensionDataStorage storage, Factory<T> factory, String name) {
        //? if >=1.20.5 {
        return storage.computeIfAbsent(new SavedData.Factory<>(factory.fallback(),
                (tag, registries) -> load(factory.codec(), tag, factory.fallback()), DataFixTypes.LEVEL), name);
        //?} else {
        /*return storage.computeIfAbsent(tag -> load(factory.codec(), tag, factory.fallback()), factory.fallback(), name);*/
        //?}
    }
